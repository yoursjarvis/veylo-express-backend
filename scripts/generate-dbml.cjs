...
function generateDbml() {
  ...
  for (const model of models) {
    ...
    for (const field of model.fields) {
      const isRelation = modelNames.has(field.type.replace('?', '').replace('[]', ''));
    ...
    if (model.compositePk) {
      const pkCols = model.compositePk.map(f => getColumnDbName(model, f)).join(', ');
      indexLines.push(`    (${pkCols}) [pk]`);
    }
    for (const uniqueGroup of model.compositeUniques) {
      const uniqueCols = uniqueGroup.map(f => getColumnDbName(model, f)).join(', ');
      indexLines.push(`    (${uniqueCols}) [unique]`);
    }
    for (const indexGroup of model.indexes) {
      const indexCols = indexGroup.map(f => getColumnDbName(model, f)).join(', ');
        ...
        if (fieldsMatch && referencesMatch) {
          ...
          if (targetModel) {
            for (let i = 0; i < fkFields.length; i++) {
              ...
              allRelations.push(`Ref: ${model.dbName}.${fkDbName} ${relSymbol} ${targetModel.dbName}.${pkDbName}${refSettings}`);
            }
          }
        }
      }
