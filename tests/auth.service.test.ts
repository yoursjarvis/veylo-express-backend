...
describe("authService.otherMethods", () => {
  ...
  it("signUp: signs up a new user, auto-logs in if email is verified", async () => {
    ...
    authApiMock.signUpEmail.mockResolvedValueOnce({
      headers,
      response: { user: { id: "user-1", email: "test@example.com", name: "Jane Doe", emailVerified: true } },
    });
    authApiMock.signInEmail.mockResolvedValueOnce({
      headers,
      response: { token: "session-token", user: { id: "user-1" } },
  ...
  it("requestPasswordReset & resetPassword", async () => {
    ...
    authApiMock.resetPassword.mockResolvedValueOnce({ headers: new Headers(), response: { success: true } });
    const res = createRes();
    const result = await authService.resetPassword(req, res, { token: "token", newPassword: "new" });
  ...
  it("verifyEmail: verifies the token and sends a welcome mail if user is found", async () => {
    prismaMockLocal.verification.findFirst.mockResolvedValueOnce({ identifier: "test@example.com", value: "token" });
  ...
  it("listSessions & revokeSession", async () => {
    ...
    authRepositoryMock.revokeSessionByIdForUser.mockResolvedValueOnce(undefined);
    prismaMockLocal.session.findFirst.mockResolvedValueOnce({ token: "token-123" });
    authApiMock.revokeSession.mockResolvedValueOnce(undefined);

    await authService.revokeSession(req, "sess-1");
    expect(authRepositoryMock.revokeSessionByIdForUser).toHaveBeenCalledWith("sess-1", "user-1");
  ...
  it("sendTwoFactorOtp & enableTwoFactorSocial", async () => {
    ...
    authApiMock.getSession.mockResolvedValueOnce({ user: { id: "user-1", email: "test@example.com" } });
    prismaMockLocal.verification.findFirst.mockResolvedValueOnce({ id: "ver-1", expiresAt: new Date(Date.now() + 100000) });
    prismaMockLocal.verification.delete.mockResolvedValueOnce({ id: "ver-1" });
    authApiMock.enableTwoFactor.mockResolvedValueOnce({ data: { success: true } });

    const res = createRes();
    const enableResult = await authService.enableTwoFactorSocial(req, res, { otp: "123456" });
...
describe("authService.edgeCases", () => {
  ...
  it("login: throws UnauthorizedException for inactive or deleted user", async () => {
    ...
    // Inactive user
    authRepositoryMock.findUserByEmail.mockResolvedValueOnce({
      id: "user-1", isActive: false, deletedAt: null, lockedUntil: null,
    });
    await expect(
      authService.login(req, res, { email: "jane@example.com", password: "pass" })
    ).rejects.toThrow("Invalid credentials");

    // Deleted user
    authRepositoryMock.findUserByEmail.mockResolvedValueOnce({
      id: "user-1", isActive: true, deletedAt: new Date(), lockedUntil: null,
    });
    await expect(
      authService.login(req, res, { email: "jane@example.com", password: "pass" })
    ).rejects.toThrow("Invalid credentials");
    ...
    // Locked user
    authRepositoryMock.findUserByEmail.mockResolvedValueOnce({
      id: "user-1", isActive: true, deletedAt: null, lockedUntil: new Date(Date.now() + 60000),
    });
    await expect(
      authService.login(req, res, { email: "jane@example.com", password: "pass" })
    ).rejects.toThrow("Invalid credentials");
  ...
  it("login: handles unexpected sign-in failures", async () => {
    authRepositoryMock.findUserByEmail.mockResolvedValueOnce({
      id: "user-1", isActive: true, deletedAt: null, lockedUntil: null,
    });
