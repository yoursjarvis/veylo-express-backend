...
describe("ObjectiveRepository", () => {
  it("should find objectives by workspace (with/without trashed)", async () => {
    ...
    prismaMock.objective.findManyWithTrashed.mockResolvedValueOnce([{ id: "obj-1" }, { id: "obj-2" }]);

    const res1 = await objectiveRepository.findObjectivesByWorkspace("ws-1", false);
    expect(prismaMock.objective.findMany).toHaveBeenCalled();
    expect(res1).toHaveLength(1);

    const res2 = await objectiveRepository.findObjectivesByWorkspace("ws-1", true);
  ...
  it("should find project by id", async () => {
    ...
    expect(prismaMock.project.findUnique).toHaveBeenCalledWith({ where: { id: "proj-1" } });
  ...
  it("should delete objective", async () => {
    ...
    expect(prismaMock.objective.delete).toHaveBeenCalledWith({ where: { id: "obj-1" } });
    expect(result.id).toBe("obj-1");
  });

  it("should find objective by id with trashed", async () => {
    prismaMock.objective.findUniqueWithTrashed.mockResolvedValueOnce({ id: "obj-1" });
    const result = await objectiveRepository.findObjectiveByIdWithTrashed("obj-1");
    expect(prismaMock.objective.findUniqueWithTrashed).toHaveBeenCalledWith({ where: { id: "obj-1" } });
    expect(result?.id).toBe("obj-1");
  });
  ...
  it("should restore objective", async () => {
    ...
    expect(prismaMock.objective.restore).toHaveBeenCalledWith({ where: { id: "obj-1" } });
    expect(result.id).toBe("obj-1");
  });
  ...
  it("should force delete objective", async () => {
    ...
    expect(prismaMock.objective.forceDelete).toHaveBeenCalledWith({ where: { id: "obj-1" } });
    expect(result.id).toBe("obj-1");
  });

  it("should update objective (with/without kr update)", async () => {
    prismaMock.objective.update.mockResolvedValueOnce({ id: "obj-1", title: "Updated" });
    ...
    prismaMock.keyResult.update.mockResolvedValueOnce({ id: "kr-1", title: "KR New" });
    prismaMock.objective.update.mockResolvedValueOnce({ id: "obj-1", title: "Updated With KR" });
