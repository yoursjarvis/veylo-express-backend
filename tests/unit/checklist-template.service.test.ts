...
describe("ChecklistTemplateService", () => {
  ...
  describe("getTemplate", () => {
    it("throws NotFoundException if template not found", async () => {
      ...
      await expect(checklistTemplateService.getTemplate("t-missing")).rejects.toThrow("Checklist template not found");
    });
  ...
  describe("updateTemplate", () => {
    it("throws NotFoundException if template not found on update", async () => {
      prismaMock.checklistTemplate.findUnique.mockResolvedValueOnce(null);
      await expect(
        checklistTemplateService.updateTemplate("t-missing", { name: "New Name" })
    ...
    it("updates template fields", async () => {
      ...
      expect(prismaMock.checklistTemplate.update).toHaveBeenCalledWith(
        expect.objectContaining({ where: { id: "t-1" }, data: { name: "Updated" } })
      );
  ...
  describe("deleteTemplate", () => {
    it("throws NotFoundException if template not found on delete", async () => {
      prismaMock.checklistTemplate.findUnique.mockResolvedValueOnce(null);
      await expect(checklistTemplateService.deleteTemplate("t-missing")).rejects.toThrow("Checklist template not found");
    });
  ...
  describe("applyTemplateToTask", () => {
    it("throws NotFoundException if template not found", async () => {
      prismaMock.checklistTemplate.findUnique.mockResolvedValueOnce(null);
      await expect(
        checklistTemplateService.applyTemplateToTask("task-1", "t-missing", "user-1")
      ).rejects.toThrow("Checklist template not found");
    });
    ...
    it("throws NotFoundException if parent task not found", async () => {
      ...
      await expect(
        checklistTemplateService.applyTemplateToTask("task-missing", "t-1", "user-1")
    ...
    it("creates subtasks from template items and uses todo status", async () => {
      ...
      const result = await checklistTemplateService.applyTemplateToTask("task-1", "t-1", "user-1");
    ...
    it("uses first status if no todo status found", async () => {
      ...
      taskRepositoryMock.incrementTaskSequence.mockResolvedValueOnce({ projectKey: "PROJ", taskSequence: 3 });
      taskExtrasRepositoryMock.createSubtask.mockResolvedValueOnce({ id: "sub-3", title: "Step 1" });
      taskExtrasRepositoryMock.createTaskActivity.mockResolvedValue(undefined);
