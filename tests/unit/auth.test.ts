...
vi.mock("@/utils/config", () => ({
  config: vi.fn((key: string) => {
    const configs: Record<string, any> = {
      ...
      "auth.betterAuth.resetPasswordRedirectURL":
...
describe("auth.ts configuration", () => {
  ...
  describe("databaseHooks", () => {
    describe("user.create.before", () => {
      it("should set emailVerified true if pending invitation exists", async () => {
        mocks.mockPrisma.invitation.findFirst.mockResolvedValueOnce({
          id: "inv-1",
        });
        const res = await config.databaseHooks.user.create.before({
          email: "test@test.com",
        });
    ...
    describe("user.create.after", () => {
      it("should send welcome email if emailVerified and auto-accept invitations", async () => {
        ...
        expect(mocks.mockMailTo).toHaveBeenCalledWith(
          "test@test.com",
          undefined,
        );
        expect(mocks.mockMailView).toHaveBeenCalledWith("welcome", {
          firstName: "John",
        });
        ...
        expect(
          mocks.mockPrisma.userRoleAssignment.create,
        ).toHaveBeenCalledTimes(2);
        expect(mocks.mockPrisma.projectMember.create).toHaveBeenCalled();
        expect(mocks.mockPrisma.invitation.update).toHaveBeenCalledWith({
          where: { id: "inv1" },
      ...
      it("should handle errors in welcome email silently", async () => {
        mocks.mockMailQueue.mockImplementationOnce(() => {
  ...
  describe("Emails", () => {
    it("sendVerificationEmail", async () => {
      ...
      await config.emailVerification.sendVerificationEmail(
        { user: { email: "a@a.com", firstName: "A" }, token: "t1" },
        mockRequest,
      );
      expect(mocks.mockMailTo).toHaveBeenCalled();
      expect(mocks.mockMailView).toHaveBeenCalledWith(
        "verify-email",
        expect.any(Object),
      );
    });
    ...
    it("sendResetPassword", async () => {
      ...
      await config.emailAndPassword.sendResetPassword(
        { user: { email: "a@a.com" }, url: "http://localhost?token=t2" },
        mockRequest,
      );
      expect(mocks.mockMailTo).toHaveBeenCalled();
      expect(mocks.mockMailView).toHaveBeenCalledWith(
  ...
  describe("Organization Plugin", () => {
    it("sendInvitationEmail", async () => {
      mocks.mockPrisma.organization.findUnique.mockResolvedValueOnce({
        slug: "org1",
        name: "Org 1",
      });
      await orgConfig.sendInvitationEmail({
        email: "test@test.com",
        organization: { id: "o1", name: "O1" },
        role: "member",
        id: "inv1",
      });
