...
describe("PortfolioService", () => {
  it("should get portfolios (active and trashed) with progress calculation", async () => {
    ...
    prismaMock.portfolio.findManyWithTrashed.mockResolvedValue(
      portfolioMockData,
    );
    ...
    const resultTrashed = await portfolioService.getPortfolios("ws-1", {
      withTrashed: true,
    });
    expect(resultTrashed).toHaveLength(1);

    const resultOnlyTrashed = await portfolioService.getPortfolios("ws-1", {
      onlyTrashed: true,
    });
  ...
  it("should get portfolio by id (active or trashed) with projects tasks mapping", async () => {
    ...
    expect(
      await portfolioService.getPortfolioById("port-1", { withTrashed: true }),
    ).toBeDefined();
  ...
  it("should update portfolio (name, description, projects lists)", async () => {
    prismaMock.portfolio.update.mockResolvedValueOnce({
      id: "port-1",
      organizationId: "org-1",
    });
  ...
  it("should delete, restore, and force delete portfolio", async () => {
    ...
    expect(await portfolioService.deletePortfolio("port-1")).toEqual({
      id: "port-1",
    });
    expect(await portfolioService.restorePortfolio("port-1")).toEqual({
      id: "port-1",
    });
    expect(await portfolioService.forceDeletePortfolio("port-1")).toEqual({
      id: "port-1",
    });
  });
});
