...
describe("DocRepository", () => {
  ...
  it("should find document by id (with & without trashed)", async () => {
    ...
    expect(await docRepository.findDocByIdWithTrashed("doc-1")).toEqual({
      id: "doc-1",
    });
  });

  it("should update document", async () => {
    prismaMock.projectDoc.update.mockResolvedValueOnce({
      id: "doc-1",
      title: "Updated",
    });
    const result = await docRepository.updateDoc("doc-1", {
      title: "Updated",
      updatedBy: "user-1",
    });
  ...
  it("should list project docs, search, and get recent docs", async () => {
    ...
    expect(await docRepository.getProjectDocs("proj-1", "user-1")).toHaveLength(
      1,
    );
    expect(
      await docRepository.searchDocs("proj-1", "query", "user-1"),
    ).toHaveLength(1);
    expect(await docRepository.getRecentDocs("proj-1", "user-1")).toHaveLength(
      1,
    );
  });
  ...
  it("should manage favorite/pin upsert and list favorites/pinned", async () => {
    ...
    const upserted = await docRepository.upsertFavorite(
      "doc-1",
      "user-1",
      "org-1",
      { isFavorite: true },
    );
    const listed = await docRepository.getFavoritesAndPinned(
      "proj-1",
      "user-1",
    );
  ...
  it("should manage document versions (create, list, findLatest, findById)", async () => {
    ...
    prismaMock.projectDocVersion.findMany.mockResolvedValueOnce([
      { id: "ver-1" },
    ]);
    prismaMock.projectDocVersion.findFirst.mockResolvedValueOnce({
      id: "ver-1",
      version: 1,
    });
    prismaMock.projectDocVersion.findUnique.mockResolvedValueOnce({
      id: "ver-1",
    });
    ...
    expect(await docRepository.findLatestVersion("doc-1")).toEqual({
      id: "ver-1",
      version: 1,
    });
    expect(await docRepository.findVersionById("ver-1")).toEqual({
      id: "ver-1",
    });
  });
  ...
  it("should manage comments (create, update, getById, delete, list)", async () => {
    ...
    prismaMock.projectDocComment.update.mockResolvedValueOnce({
      id: "c-1",
      content: "New",
    });
    prismaMock.projectDocComment.findUnique.mockResolvedValueOnce({
      id: "c-1",
    });
    prismaMock.projectDocComment.delete.mockResolvedValueOnce({ id: "c-1" });
    prismaMock.projectDocComment.findMany.mockResolvedValueOnce([
      { id: "c-1" },
    ]);

    expect(
      await docRepository.createComment({
        docId: "doc-1",
        userId: "user-1",
        organizationId: "org-1",
        content: "comment",
    ...
    expect(
      await docRepository.updateComment("c-1", { content: "New" }),
    ).toEqual({ id: "c-1", content: "New" });
  ...
  it("should manage document activities", async () => {
    ...
    prismaMock.projectDocActivity.findMany.mockResolvedValueOnce([
      { id: "act-1" },
    ]);
  ...
  it("should manage document custom permissions", async () => {
    prismaMock.projectDocPermission.upsert.mockResolvedValueOnce({
      id: "perm-1",
    });
    prismaMock.projectDocPermission.deleteMany.mockResolvedValueOnce({
      count: 1,
    });
    prismaMock.projectDocPermission.findMany.mockResolvedValueOnce([
      { id: "perm-1" },
    ]);

    expect(
      await docRepository.updateDocPermission(
        "doc-1",
        "user-1",
        "org-1",
        "edit",
    ...
    expect(
      await docRepository.deleteDocPermission("doc-1", "user-1"),
    ).toBeDefined();
  ...
  it("should toggle comment reaction (create if not exist, delete if exist)", async () => {
    ...
    prismaMock.projectDocCommentReaction.findUnique.mockResolvedValueOnce({
      id: "re-1",
    });
    prismaMock.projectDocCommentReaction.delete.mockResolvedValueOnce({
      id: "re-1",
    });
    const res1 = await docRepository.toggleReaction({
      commentId: "c-1",
      userId: "user-1",
      organizationId: "org-1",
      emoji: "smile",
    ...
    prismaMock.projectDocCommentReaction.create.mockResolvedValueOnce({
      id: "re-2",
    });
    const res2 = await docRepository.toggleReaction({
      commentId: "c-1",
      userId: "user-1",
      organizationId: "org-1",
      emoji: "smile",
