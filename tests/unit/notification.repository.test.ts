...
describe("NotificationRepository", () => {
  it("should get notifications for a recipient", async () => {
    prismaMock.notification.findMany.mockResolvedValueOnce([{ id: "notif-1", recipientId: "user-1" }]);
  ...
  it("should find notification by id", async () => {
    ...
    expect(prismaMock.notification.findUnique).toHaveBeenCalledWith({ where: { id: "notif-1" } });
    expect(result?.id).toBe("notif-1");
  });

  it("should mark notification as read", async () => {
    prismaMock.notification.update.mockResolvedValueOnce({ id: "notif-1", isRead: true });
  ...
  it("should find active slack webhooks", async () => {
    prismaMock.slackWebhook.findMany.mockResolvedValueOnce([{ id: "webhook-1" }]);
    const result = await notificationRepository.findActiveSlackWebhooks("proj-1");
  ...
  it("should create notification", async () => {
    ...
    prismaMock.notification.create.mockResolvedValueOnce({ id: "notif-1", ...data });
  ...
  it("should find task for notification", async () => {
    prismaMock.task.findUnique.mockResolvedValueOnce({ id: "task-1", title: "Task 1" });
    const result = await notificationRepository.findTaskForNotification("task-1");
  ...
  it("should find user name and notification preferences", async () => {
    ...
    prismaMock.user.findUnique.mockResolvedValueOnce({ notificationPreferences: {} });

    expect(await notificationRepository.findUserName("user-1")).toEqual({ name: "John Doe" });
    expect(await notificationRepository.getUserPreferences("user-1")).toEqual({ notificationPreferences: {} });
  });

  it("should find comment for notification", async () => {
    prismaMock.comment.findUnique.mockResolvedValueOnce({ id: "comment-1" });
    const result = await notificationRepository.findCommentForNotification("comment-1");
    expect(prismaMock.comment.findUnique).toHaveBeenCalled();
    expect(result?.id).toBe("comment-1");
  });

  it("should find project members", async () => {
    prismaMock.projectMember.findMany.mockResolvedValueOnce([{ id: "member-1" }]);
  ...
  it("should find project for notification", async () => {
    prismaMock.project.findUnique.mockResolvedValueOnce({ id: "proj-1", title: "Project" });
    const result = await notificationRepository.findProjectForNotification("proj-1");
  ...
  it("should find comment reaction for notification", async () => {
    ...
    const result = await notificationRepository.findReactionForNotification("re-1");
