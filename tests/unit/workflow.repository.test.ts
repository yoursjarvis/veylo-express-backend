...
describe("WorkflowRepository", () => {
  it("should find transition", async () => {
    prismaMock.workflowTransition.findUnique.mockResolvedValueOnce({
      id: "tr-1",
    });
    const result = await workflowRepository.findTransition(
      "proj-1",
      "status-1",
      "status-2",
    );
  ...
  it("should create transition", async () => {
    ...
    prismaMock.workflowTransition.create.mockResolvedValueOnce({
      id: "tr-1",
      ...data,
    });
  ...
  it("should delete transition", async () => {
    ...
    expect(prismaMock.workflowTransition.delete).toHaveBeenCalledWith({
      where: { id: "tr-1" },
    });
    expect(result.id).toBe("tr-1");
  });
  ...
  it("should get transitions by project", async () => {
    prismaMock.workflowTransition.findMany.mockResolvedValueOnce([
      { id: "tr-1" },
    ]);
  ...
  it("should get transitions from status", async () => {
    ...
    const result = await workflowRepository.getTransitionsFromStatus(
      "proj-1",
      "status-1",
    );
    expect(prismaMock.workflowTransition.findMany).toHaveBeenCalledWith({
