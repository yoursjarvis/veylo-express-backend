...
describe("ProjectService", () => {
  ...
  describe("createProject", () => {
    ...
    it("creates project with DB template (config statuses)", async () => {
      ...
      projectRepositoryMock.findWorkspaceById.mockResolvedValueOnce({
        organizationId: "org-1",
      });
      projectRepositoryMock.createProject.mockResolvedValueOnce({
        id: "proj-1",
        organizationId: "org-1",
        ownerId: "user-1",
      });
  ...
  describe("getProjectTemplateBySlug", () => {
    it("throws NotFoundException if template not found", async () => {
      projectRepositoryMock.findTemplateBySlug.mockResolvedValueOnce(null);
      await expect(
        projectService.getProjectTemplateBySlug("missing"),
  ...
  describe("getProject", () => {
    it("throws NotFoundException if project not found", async () => {
      ...
      await expect(projectService.getProject("proj-missing")).rejects.toThrow(
        "Project not found",
      );
    });

    it("returns project details", async () => {
      projectRepositoryMock.getProjectDetails.mockResolvedValueOnce({
        id: "proj-1",
      });
  ...
  describe("getProjects / getOrgProjects", () => {
    it("getProjects delegates to repository", async () => {
      projectRepositoryMock.getProjects.mockResolvedValueOnce([
        { id: "proj-1" },
      ]);
    ...
    it("getOrgProjects checks RBAC then delegates", async () => {
      ...
      projectRepositoryMock.getOrgProjects.mockResolvedValueOnce([
        { id: "proj-1" },
      ]);
      const result = await projectService.getOrgProjects("org-1", "user-1");
      expect(result).toHaveLength(1);
      expect(rbacServiceMock.authorize).toHaveBeenCalledWith(
        "user-1",
        "project:read",
        expect.any(Object),
      );
  ...
  describe("updateProject / deleteProject", () => {
    it("updates project", async () => {
      projectRepositoryMock.updateProject.mockResolvedValueOnce({
        id: "proj-1",
        title: "Updated",
      });
    ...
    it("deletes project", async () => {
      projectRepositoryMock.deleteProject.mockResolvedValueOnce({
        id: "proj-1",
      });
  ...
  describe("restoreProject / forceDeleteProject", () => {
    it("throws NotFoundException for restore if project not found", async () => {
      projectRepositoryMock.findProjectByIdWithTrashed.mockResolvedValueOnce(
        null,
      );
      await expect(
        projectService.restoreProject("proj-missing"),
    ...
    it("restores a project", async () => {
      ...
      projectRepositoryMock.restoreProject.mockResolvedValueOnce({
        id: "proj-1",
      });
    ...
    it("throws NotFoundException for force delete if project not found", async () => {
      ...
      await expect(
        projectService.forceDeleteProject("proj-missing"),
      ).rejects.toThrow("Project not found");
    });
    ...
    it("force deletes a project", async () => {
      ...
      projectRepositoryMock.forceDeleteProject.mockResolvedValueOnce({
        id: "proj-1",
      });
  ...
  describe("addProjectMembers", () => {
    ...
    it("adds members and triggers notification", async () => {
      ...
      projectRepositoryMock.upsertProjectMember.mockResolvedValue({
        id: "pm-1",
      });
      notificationServiceMock.handleAddedToProject.mockResolvedValueOnce(
        undefined,
      );
      ...
      expect(notificationServiceMock.handleAddedToProject).toHaveBeenCalledWith(
        "proj-1",
        "adder-1",
        ["user-1", "user-2"],
      );
  ...
  describe("getProjectMembers / removeProjectMember", () => {
    it("gets project members", async () => {
      projectRepositoryMock.getProjectMembers.mockResolvedValueOnce([
        { id: "pm-1" },
      ]);
    ...
    it("removes a project member", async () => {
      projectRepositoryMock.deleteProjectMember.mockResolvedValueOnce({
        id: "pm-1",
      });
  ...
  describe("getProjectVault", () => {
    it("creates vault if not existing", async () => {
      ...
      projectRepositoryMock.createVault.mockResolvedValueOnce({
        id: "vault-1",
        services: [],
      });
  ...
  describe("addVaultService", () => {
    it("throws NotFoundException if vault not found", async () => {
      projectRepositoryMock.findVault.mockResolvedValueOnce(null);
      await expect(
        projectService.addVaultService("proj-1", "Stripe"),
    ...
    it("throws BadRequestException if service already exists", async () => {
      ...
      projectRepositoryMock.findVaultService.mockResolvedValueOnce({
        id: "svc-existing",
      });
      await expect(
        projectService.addVaultService("proj-1", "Stripe"),
    ...
    it("creates vault service successfully", async () => {
      ...
      projectRepositoryMock.createVaultService.mockResolvedValueOnce({
        id: "svc-1",
      });
  ...
  describe("restoreVaultService / forceDeleteVaultService", () => {
    it("throws NotFoundException for restore if service not found", async () => {
      projectRepositoryMock.findVaultServiceByIdWithTrashed.mockResolvedValueOnce(
        null,
      );
      await expect(
        projectService.restoreVaultService("svc-missing"),
    ...
    it("restores vault service", async () => {
      ...
      projectRepositoryMock.restoreVaultService.mockResolvedValueOnce({
        id: "svc-1",
      });
    ...
    it("throws NotFoundException for force delete if service not found", async () => {
      ...
      await expect(
        projectService.forceDeleteVaultService("svc-missing"),
      ).rejects.toThrow("Vault service not found");
    });
    ...
    it("force deletes vault service", async () => {
      ...
      projectRepositoryMock.forceDeleteVaultService.mockResolvedValueOnce({
        id: "svc-1",
      });
  ...
  describe("addOrUpdateVaultItem / updateVaultItem", () => {
    ...
    it("updates vault item values", async () => {
      ...
      const result = await projectService.updateVaultItem("item-1", {
        value: "new-secret",
        note: null,
      });
  ...
  describe("restoreVaultItem / forceDeleteVaultItem", () => {
    it("throws NotFoundException for restore if item not found", async () => {
      projectRepositoryMock.findVaultItemByIdWithTrashed.mockResolvedValueOnce(
        null,
      );
      await expect(
        projectService.restoreVaultItem("item-missing"),
      ).rejects.toThrow("Vault item not found");
    });
    ...
    it("restores vault item", async () => {
      ...
      projectRepositoryMock.restoreVaultItem.mockResolvedValueOnce({
        id: "item-1",
      });
    ...
    it("throws NotFoundException for force delete if item not found", async () => {
      ...
      await expect(
        projectService.forceDeleteVaultItem("item-missing"),
      ).rejects.toThrow("Vault item not found");
    });
    ...
    it("force deletes vault item", async () => {
      ...
      projectRepositoryMock.forceDeleteVaultItem.mockResolvedValueOnce({
        id: "item-1",
      });
  ...
  describe("uploadProjectFile", () => {
    it("throws BadRequestException for disallowed file extension", async () => {
      ...
      await expect(
        projectService.uploadProjectFile("proj-1", file),
  ...
  describe("getProjectFiles", () => {
    it("returns project files with URLs", async () => {
      ...
      mediaServiceMock.generateUrl.mockReturnValue(
  ...
  describe("deleteProjectFile", () => {
    it("throws NotFoundException if file not found", async () => {
      ...
      await expect(
        projectService.deleteProjectFile("proj-1", "file-missing"),
      ).rejects.toThrow("File not found in this project");
    });

    it("deletes a project file", async () => {
      projectRepositoryMock.findProjectFile.mockResolvedValueOnce({
        id: "file-1",
      });
  ...
  describe("getAutomationRules / createAutomationRule", () => {
    it("returns automation rules", async () => {
      projectRepositoryMock.findAutomationRules.mockResolvedValueOnce([
        { id: "rule-1" },
      ]);
    ...
    it("throws BadRequestException if rule data is incomplete", () => {
      expect(
        () =>
          projectService.createAutomationRule("proj-1", {
            name: "Rule",
    ...
    it("creates automation rule", async () => {
      projectRepositoryMock.createAutomationRule.mockResolvedValueOnce({
        id: "rule-1",
      });
  ...
  describe("updateAutomationRule / deleteAutomationRule", () => {
    it("throws NotFoundException when updating nonexistent rule", async () => {
      ...
      await expect(
        projectService.updateAutomationRule("rule-missing", {}),
      ).rejects.toThrow("Automation rule not found");
    });

    it("updates automation rule", async () => {
      projectRepositoryMock.findAutomationRuleById.mockResolvedValueOnce({
        id: "rule-1",
      });
      projectRepositoryMock.updateAutomationRule.mockResolvedValueOnce({
        id: "rule-1",
      });
      const result = await projectService.updateAutomationRule("rule-1", {
        isActive: false,
      });
      expect(result).toEqual({ id: "rule-1" });
    });
    ...
    it("throws NotFoundException when deleting nonexistent rule", async () => {
      projectRepositoryMock.findAutomationRuleById.mockResolvedValueOnce(null);
      await expect(
        projectService.deleteAutomationRule("rule-missing"),
      ).rejects.toThrow("Automation rule not found");
    });
    ...
    it("deletes automation rule", async () => {
      ...
      projectRepositoryMock.deleteAutomationRule.mockResolvedValueOnce({
        id: "rule-1",
      });
  ...
  describe("restoreAutomationRule / forceDeleteAutomationRule", () => {
    it("throws NotFoundException for restore if rule not found", async () => {
      projectRepositoryMock.findAutomationRuleByIdWithTrashed.mockResolvedValueOnce(
        null,
      );
      await expect(
        projectService.restoreAutomationRule("rule-missing"),
      ).rejects.toThrow("Automation rule not found");
    });
    ...
    it("restores automation rule", async () => {
      ...
      projectRepositoryMock.restoreAutomationRule.mockResolvedValueOnce({
        id: "rule-1",
      });
    ...
    it("throws NotFoundException for force delete if rule not found", async () => {
      ...
      await expect(
        projectService.forceDeleteAutomationRule("rule-missing"),
      ).rejects.toThrow("Automation rule not found");
    });
    ...
    it("force deletes automation rule", async () => {
      ...
      projectRepositoryMock.forceDeleteAutomationRule.mockResolvedValueOnce({
        id: "rule-1",
      });
