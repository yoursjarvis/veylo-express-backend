...
describe("TaskExtrasRepository", () => {
  it("should find task by id", async () => {
    ...
    expect(prismaMock.task.findUnique).toHaveBeenCalledWith({ where: { id: "task-1" } });
  ...
  it("should find status by name and project id", async () => {
    ...
    const result = await taskExtrasRepository.findStatusByNameAndProjectId("Todo", "proj-1");
  ...
  it("should create status", async () => {
    ...
    prismaMock.taskStatus.create.mockResolvedValueOnce({ id: "status-1", ...data });
  ...
  it("should update status", async () => {
    prismaMock.taskStatus.update.mockResolvedValueOnce({ id: "status-1", name: "New" });
    const result = await taskExtrasRepository.updateStatus("status-1", { name: "New" });
  ...
  it("should find status by id with trashed, restore, forceDelete", async () => {
    prismaMock.taskStatus.findUniqueWithTrashed.mockResolvedValueOnce({ id: "status-1" });
    ...
    expect((await taskExtrasRepository.findStatusByIdWithTrashed("status-1"))?.id).toBe("status-1");
    expect((await taskExtrasRepository.restoreStatus("status-1")).id).toBe("status-1");
    expect((await taskExtrasRepository.forceDeleteStatus("status-1")).id).toBe("status-1");
  });
  ...
  it("should find, update, and delete subtask", async () => {
    ...
    prismaMock.task.update.mockResolvedValueOnce({ id: "subtask-1", title: "Updated Sub" });
    prismaMock.task.delete.mockResolvedValueOnce({ id: "subtask-1" });

    expect((await taskExtrasRepository.findSubtaskById("subtask-1"))?.id).toBe("subtask-1");
    expect((await taskExtrasRepository.updateSubtask("subtask-1", { title: "Updated Sub" })).title).toBe("Updated Sub");
    expect((await taskExtrasRepository.deleteSubtask("subtask-1")).id).toBe("subtask-1");
  });
  ...
  it("should create task activity", async () => {
    ...
    prismaMock.taskActivity.create.mockResolvedValueOnce({ id: "act-1", ...data });
  ...
  it("should find, delete comment, find with trashed, restore, forceDelete, and update comment", async () => {
    ...
    prismaMock.comment.findUniqueWithTrashed.mockResolvedValueOnce({ id: "c-1" });
    prismaMock.comment.restore.mockResolvedValueOnce({ id: "c-1" });
    prismaMock.comment.forceDelete.mockResolvedValueOnce({ id: "c-1" });
    prismaMock.comment.update.mockResolvedValueOnce({ id: "c-1", content: "New Content" });
    ...
    expect((await taskExtrasRepository.findCommentByIdWithTrashed("c-1"))?.id).toBe("c-1");
    expect((await taskExtrasRepository.restoreComment("c-1")).id).toBe("c-1");
    expect((await taskExtrasRepository.forceDeleteComment("c-1")).id).toBe("c-1");
    expect((await taskExtrasRepository.updateComment("c-1", "New Content")).content).toBe("New Content");
  });
  ...
  it("should find org member and workspace member", async () => {
    ...
    expect((await taskExtrasRepository.findOrgMember("org-1", "user-1"))?.id).toBe("m-1");
    expect((await taskExtrasRepository.findWorkspaceMember("ws-1", "user-1"))?.id).toBe("wm-1");
  });

  // --- CUSTOM FIELDS TESTS ---
  it("should find custom field by name, create, find many, find by id, delete, find with trashed, restore, and forceDelete", async () => {
    prismaMock.customFieldDefinition.findFirst.mockResolvedValueOnce({ id: "cf-1" });
    ...
    prismaMock.customFieldDefinition.findUnique.mockResolvedValueOnce({ id: "cf-1" });
    prismaMock.customFieldDefinition.delete.mockResolvedValueOnce({ id: "cf-1" });
    prismaMock.customFieldDefinition.findUniqueWithTrashed.mockResolvedValueOnce({ id: "cf-1" });
    ...
    expect((await taskExtrasRepository.findCustomFieldByName("CF", "proj-1"))?.id).toBe("cf-1");
    expect((await taskExtrasRepository.createCustomField({
      name: "CF",
      type: "text",
      projectId: "proj-1",
      organizationId: "org-1",
    })).id).toBe("cf-1");
    expect(await taskExtrasRepository.findCustomFieldsByProjectId("proj-1")).toHaveLength(1);
    expect((await taskExtrasRepository.findCustomFieldById("cf-1"))?.id).toBe("cf-1");
    expect((await taskExtrasRepository.deleteCustomField("cf-1")).id).toBe("cf-1");
    expect((await taskExtrasRepository.findCustomFieldByIdWithTrashed("cf-1"))?.id).toBe("cf-1");
    expect((await taskExtrasRepository.restoreCustomField("cf-1")).id).toBe("cf-1");
    expect((await taskExtrasRepository.forceDeleteCustomField("cf-1")).id).toBe("cf-1");
  });

  // --- REACTIONS TESTS ---
  it("should find comment reactions", async () => {
    prismaMock.commentReaction.findMany.mockResolvedValueOnce([{ id: "r-1" }]);
    const result = await taskExtrasRepository.findCommentReactions("c-1", "smile");
    expect(result).toHaveLength(1);
  });

  it("should find specific comment reaction", async () => {
    prismaMock.comment.findUnique.mockResolvedValueOnce({ organizationId: "org-1" });
    prismaMock.commentReaction.findUnique.mockResolvedValueOnce({ id: "r-1" });

    const result = await taskExtrasRepository.findCommentReaction("c-1", "user-1", "smile");
  ...
  it("should create comment reaction if comment exists", async () => {
    ...
    const result = await taskExtrasRepository.createCommentReaction("c-1", "user-1", "smile");
    expect(result.id).toBe("r-1");
  });
