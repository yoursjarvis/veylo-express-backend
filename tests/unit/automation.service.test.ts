...
describe("AutomationService", () => {
  ...
  it("should handle handleTaskCreated", async () => {
    const task = { id: "task-1", projectId: "proj-1", creatorId: "user-1", organizationId: "org-1" };
    prismaMock.task.findUnique.mockResolvedValueOnce(task as any);

    const rule = { id: "rule-1", name: "R1", trigger: "task_created", action: "add_comment", actionVal: "Auto comment" };
    prismaMock.automationRule.findMany.mockResolvedValueOnce([rule as any]);
  ...
  it("should handle handleTaskStatusChanged with trigger matches", async () => {
    ...
    const rules = [
      { id: "rule-1", name: "R1", trigger: "task_status_changed", triggerVal: "Todo", action: "assign_to_creator" },
      { id: "rule-2", name: "R2", trigger: "task_status_changed", triggerVal: "todo", action: "set_priority", actionVal: "high" },
      { id: "rule-3", name: "R3", trigger: "task_status_changed", triggerVal: "Done", action: "assign_to_creator" },
    ];
    prismaMock.automationRule.findMany.mockResolvedValueOnce(rules as any);
    ...
    await automationService.handleTaskStatusChanged("task-1", "user-1", "Backlog", "Todo");
  ...
  it("should handle handlePriorityChanged", async () => {
    ...
    const rules = [
      { id: "rule-1", name: "R1", trigger: "priority_changed", triggerVal: "high", action: "close_parent" },
    ];
    prismaMock.automationRule.findMany.mockResolvedValueOnce(rules as any);
    ...
    await automationService.handlePriorityChanged("task-1", "user-1", "medium", "high");
  ...
  it("should handle handleSubtaskCompleted when all are done", async () => {
    ...
    prismaMock.task.findUnique.mockResolvedValueOnce(parentTask as any);

    const rules = [
      { id: "rule-1", name: "R1", trigger: "subtasks_all_done", action: "assign_to_user", actionVal: "user-assignee" },
    ];
    prismaMock.automationRule.findMany.mockResolvedValueOnce(rules as any);
    ...
    expect(taskService.updateTask).toHaveBeenCalledWith("parent-1", "user-1", { assigneeId: "user-assignee" });
  });
  ...
  it("should return early if task not found in handleTaskStatusChanged", async () => {
    ...
    await automationService.handleTaskStatusChanged("task-missing", "user-1", "Todo", "Done");
  ...
  it("should return early if task not found in handlePriorityChanged", async () => {
    ...
    await automationService.handlePriorityChanged("task-missing", "user-1", "low", "high");
  ...
  it("should skip executeAction for update_status when task status already matches", async () => {
    const task = {
      id: "task-1", projectId: "proj-1", statusId: "status-done",
      title: "T1", creatorId: "user-1", organizationId: "org-1",
      priority: "medium", assigneeId: null,
    };
    prismaMock.task.findUnique.mockResolvedValueOnce(task as any);

    const rules = [
      { id: "rule-1", name: "R1", trigger: "task_created", action: "update_status", actionVal: "Done" },
    ];
    prismaMock.automationRule.findMany.mockResolvedValueOnce(rules as any);
    prismaMock.taskStatus.findMany.mockResolvedValueOnce([
      { id: "status-done", name: "Done", category: "done" },
  ...
  it("should skip assign_to_creator if already assigned", async () => {
    const task = {
      id: "task-1", projectId: "proj-1", creatorId: "user-creator",
      assigneeId: "user-creator", // already assigned to creator
      title: "T1", organizationId: "org-1", priority: "medium",
    };
    prismaMock.task.findUnique.mockResolvedValueOnce(task as any);

    const rules = [
      { id: "rule-1", name: "R1", trigger: "task_created", action: "assign_to_creator" },
    ];
    prismaMock.automationRule.findMany.mockResolvedValueOnce(rules as any);
  ...
  it("should skip set_priority if priority already matches", async () => {
    const task = {
      id: "task-1", projectId: "proj-1", priority: "high",
      title: "T1", organizationId: "org-1", creatorId: "user-1", assigneeId: null,
    };
    prismaMock.task.findUnique.mockResolvedValueOnce(task as any);

    const rules = [
      { id: "rule-1", name: "R1", trigger: "task_created", action: "set_priority", actionVal: "high" },
    ];
    prismaMock.automationRule.findMany.mockResolvedValueOnce(rules as any);
  ...
  it("should skip handlePriorityChanged if old and new priority are the same", async () => {
    ...
    await automationService.handlePriorityChanged("task-1", "user-1", "high", "high");
  ...
  it("should execute add_comment action with default content when actionVal is missing", async () => {
    const task = {
      id: "task-1", projectId: "proj-1", creatorId: "user-1",
      organizationId: "org-1", priority: "medium",
    };
    prismaMock.task.findUnique.mockResolvedValueOnce(task as any);

    const rules = [
      { id: "rule-1", name: "R1", trigger: "task_created", action: "add_comment", actionVal: null },
    ];
    prismaMock.automationRule.findMany.mockResolvedValueOnce(rules as any);
    ...
    expect(prismaMock.comment.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ content: "Automated action executed." }) })
    );
  });
  ...
  it("should catch and log errors in executeAction", async () => {
    const task = {
      id: "task-1", projectId: "proj-1", creatorId: "user-1",
      organizationId: "org-1", priority: "medium", title: "T1",
    };
    prismaMock.task.findUnique.mockResolvedValueOnce(task as any);
    ...
    prismaMock.automationRule.findMany.mockResolvedValueOnce(rules as any);
    prismaMock.taskStatus.findMany.mockRejectedValueOnce(new Error("DB status error"));
  ...
  it("should use category fallback when status name doesn't match but category matches done", async () => {
    const task = {
      id: "task-1", projectId: "proj-1", statusId: "status-progress",
      title: "T1", creatorId: "user-1", organizationId: "org-1",
      priority: "medium", assigneeId: null,
    };
    prismaMock.task.findUnique.mockResolvedValueOnce(task as any);
    ...
    const rules = [
      { id: "rule-1", name: "R1", trigger: "task_created", action: "update_status", actionVal: "Completed" },
    ];
    prismaMock.automationRule.findMany.mockResolvedValueOnce(rules as any);
    ...
    expect(taskService.updateTask).toHaveBeenCalledWith("task-1", "user-1", { statusId: "status-done" });
  });
