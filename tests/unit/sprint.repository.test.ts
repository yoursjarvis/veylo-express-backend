...
describe("SprintRepository", () => {
  ...
  it("should find sprint by id", async () => {
    ...
    expect(prismaMock.sprint.findUnique).toHaveBeenCalledWith({ where: { id: "sprint-1" } });
    expect(result?.id).toBe("sprint-1");
  });

  it("should find sprint by id with tasks", async () => {
    prismaMock.sprint.findUnique.mockResolvedValueOnce({ id: "sprint-1", tasks: [] });
  ...
  it("should find first active sprint by projectId", async () => {
    prismaMock.sprint.findFirst.mockResolvedValueOnce({ id: "sprint-1", status: "active" });
  ...
  it("should find sprint in project", async () => {
    prismaMock.sprint.findFirst.mockResolvedValueOnce({ id: "sprint-1", projectId: "proj-1" });
    const result = await sprintRepository.findSprintInProject("sprint-1", "proj-1");
    expect(prismaMock.sprint.findFirst).toHaveBeenCalledWith({
  ...
  it("should find uncompleted tasks in sprint", async () => {
    ...
    const result = await sprintRepository.findUncompletedTasksInSprint("sprint-1");
  ...
  it("should update tasks sprint", async () => {
    ...
    const result = await sprintRepository.updateTasksSprint(["task-1", "task-2"], "sprint-2");
  ...
  it("should create task activities", async () => {
    ...
    expect(prismaMock.taskActivity.createMany).toHaveBeenCalledWith({ data: payloads });
    expect(result.count).toBe(1);
  });

  it("should update sprint", async () => {
    prismaMock.sprint.update.mockResolvedValueOnce({ id: "sprint-1", name: "New Name" });
    const result = await sprintRepository.update("sprint-1", { name: "New Name" });
  ...
  it("should delete sprint", async () => {
    ...
    expect(prismaMock.sprint.delete).toHaveBeenCalledWith({ where: { id: "sprint-1" } });
    expect(result.id).toBe("sprint-1");
  });

  it("should find sprint by id with trashed", async () => {
    prismaMock.sprint.findUniqueWithTrashed.mockResolvedValueOnce({ id: "sprint-1" });
    const result = await sprintRepository.findByIdWithTrashed("sprint-1");
    expect(prismaMock.sprint.findUniqueWithTrashed).toHaveBeenCalledWith({ where: { id: "sprint-1" } });
    expect(result?.id).toBe("sprint-1");
  });
  ...
  it("should restore sprint", async () => {
    ...
    expect(prismaMock.sprint.restore).toHaveBeenCalledWith({ where: { id: "sprint-1" } });
    expect(result.id).toBe("sprint-1");
  });
  ...
  it("should force delete sprint", async () => {
    ...
    expect(prismaMock.sprint.forceDelete).toHaveBeenCalledWith({ where: { id: "sprint-1" } });
    expect(result.id).toBe("sprint-1");
  });
});
