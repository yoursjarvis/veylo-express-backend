...
describe("WorkspaceRepository", () => {
  it("should find workspace by id and org, by slug, and by slug exclude id", async () => {
    ...
    prismaMock.workspace.findUnique.mockResolvedValueOnce({ id: "ws-1", slug: "ws-slug" });
    prismaMock.workspace.findFirst.mockResolvedValueOnce({ id: "ws-2" });

    expect(await workspaceRepository.findWorkspaceByIdAndOrg("ws-1", "org-1")).toEqual({ id: "ws-1" });
    expect(await workspaceRepository.findWorkspaceBySlug("ws-slug")).toEqual({ id: "ws-1", slug: "ws-slug" });
    expect(await workspaceRepository.findWorkspaceBySlugExcludeId("ws-slug", "ws-1")).toEqual({ id: "ws-2" });
  });
  ...
  it("should find org member, workspace member, and workspace member with org", async () => {
    ...
    expect(await workspaceRepository.findOrgMember("org-1", "user-1")).toEqual({ id: "m-1" });
    expect(await workspaceRepository.findWorkspaceMember("ws-1", "user-1")).toEqual({ id: "wm-1" });
    expect(await workspaceRepository.findWorkspaceMemberWithOrg("ws-1", "user-1", "org-1")).toEqual({ id: "wm-2" });
  });

  it("should get workspaces for user", async () => {
    prismaMock.workspace.findMany.mockResolvedValueOnce([{ id: "ws-1" }]);
    const result = await workspaceRepository.getWorkspacesForUser("org-1", "user-1");
    expect(prismaMock.workspace.findMany).toHaveBeenCalled();
    expect(result).toHaveLength(1);
  });

  it("should create, update, and delete workspace", async () => {
    const data = { name: "WS 1", slug: "ws-1", organizationId: "org-1", creatorUserId: "user-1" };
    prismaMock.workspace.create.mockResolvedValueOnce({ id: "ws-1" });
    prismaMock.workspace.update.mockResolvedValueOnce({ id: "ws-1", name: "New Name" });
    prismaMock.workspace.delete.mockResolvedValueOnce({ id: "ws-1" });

    expect(await workspaceRepository.createWorkspace(data)).toEqual({ id: "ws-1" });
    expect(await workspaceRepository.updateWorkspace("ws-1", { name: "New Name" })).toEqual({ id: "ws-1", name: "New Name" });
    expect(await workspaceRepository.deleteWorkspace("ws-1")).toEqual({ id: "ws-1" });
  });

  it("should find by id with trashed, restore, and forceDelete workspace", async () => {
    prismaMock.workspace.findUniqueWithTrashed.mockResolvedValueOnce({ id: "ws-1" });
    prismaMock.workspace.restore.mockResolvedValueOnce({ id: "ws-1" });
    prismaMock.workspace.forceDelete.mockResolvedValueOnce({ id: "ws-1" });

    expect(await workspaceRepository.findWorkspaceByIdWithTrashed("ws-1")).toEqual({ id: "ws-1" });
    expect(await workspaceRepository.restoreWorkspace("ws-1")).toEqual({ id: "ws-1" });
    expect(await workspaceRepository.forceDeleteWorkspace("ws-1")).toEqual({ id: "ws-1" });
  });
  ...
  it("should sync org admins to workspaces", async () => {
    ...
    prismaMock.member.findMany.mockResolvedValueOnce([{ userId: "user-1" }, { userId: "user-2" }]);
    prismaMock.workspace.findMany.mockResolvedValueOnce([{ id: "ws-1" }, { id: "ws-2" }]);
  ...
  it("should get workspace members and org members by ids", async () => {
    ...
    expect(await workspaceRepository.getWorkspaceMembers("ws-1", "org-1")).toEqual([{ id: "wm-1" }]);
    expect(await workspaceRepository.getOrgMembersByIds("org-1", ["user-1"])).toEqual([{ id: "m-1" }]);
  });
  ...
  it("should upsert workspace member", async () => {
    ...
    const result = await workspaceRepository.upsertWorkspaceMember("ws-1", "user-1");
  ...
  it("should delete workspace member", async () => {
    ...
    const result = await workspaceRepository.deleteWorkspaceMember("ws-1", "user-1");
    expect(result).toEqual({ id: "wm-1" });
  });
});
