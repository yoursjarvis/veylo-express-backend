...
describe("SprintService", () => {
  ...
  describe("getSprints & getSprint", () => {
    it("returns sprints for a project", async () => {
      sprintRepositoryMock.findByProjectId.mockResolvedValueOnce([
        { id: "sprint-1" },
      ]);
    ...
    it("returns sprint with tasks by ID", async () => {
      sprintRepositoryMock.findByIdWithTasks.mockResolvedValueOnce({
        id: "sprint-1",
        tasks: [],
      });
  ...
  describe("updateSprint", () => {
    it("updates sprint name, goal, and clears dates to null", async () => {
      ...
      expect(sprintRepositoryMock.update).toHaveBeenCalledWith(
        "sprint-1",
        expect.objectContaining({
          name: "Sprint 1 Updated",
          goal: null,
    ...
    it("sets status to active when no active sprint exists", async () => {
      sprintRepositoryMock.findFirstActiveByProjectId.mockResolvedValueOnce(
        null,
      );
      sprintRepositoryMock.update.mockResolvedValueOnce({
        id: "sprint-1",
        status: "active",
      });
      ...
      expect(
        sprintRepositoryMock.findFirstActiveByProjectId,
    ...
    it("completes sprint and moves uncompleted tasks to destination sprint", async () => {
      ...
      sprintRepositoryMock.findSprintInProject.mockResolvedValueOnce(
        destSprint,
      );
      ...
      sprintRepositoryMock.createTaskActivities.mockResolvedValueOnce(
        undefined,
      );
      sprintRepositoryMock.update.mockResolvedValueOnce({
        id: "sprint-1",
      ...
      expect(sprintRepositoryMock.findSprintInProject).toHaveBeenCalledWith(
        "sprint-dest",
        "proj-1",
      );
  ...
  describe("restoreSprint", () => {
    it("throws NotFoundException if sprint not found", async () => {
      ...
      await expect(
        sprintService.restoreSprint("sprint-missing"),
      ).rejects.toThrow("Sprint not found");
    });

    it("restores a soft-deleted sprint", async () => {
      sprintRepositoryMock.findByIdWithTrashed.mockResolvedValueOnce({
        id: "sprint-1",
      });
  ...
  describe("forceDeleteSprint", () => {
    it("throws NotFoundException if sprint not found", async () => {
      sprintRepositoryMock.findByIdWithTrashed.mockResolvedValueOnce(null);
      await expect(
        sprintService.forceDeleteSprint("sprint-missing"),
      ).rejects.toThrow("Sprint not found");
    });
    ...
    it("permanently deletes a sprint", async () => {
      ...
      sprintRepositoryMock.forceDelete.mockResolvedValueOnce({
        id: "sprint-1",
      });
