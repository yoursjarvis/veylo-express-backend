...
const { configMock } = vi.hoisted(() => ({
  configMock: vi.fn().mockImplementation((key: string) => {
    if (key === "app.vaultEncryptionKey")
      return "test-secret-key-must-be-long-enough-32-chars";
    return undefined;
  }),
}));
...
describe("crypto helper utilities", () => {
  ...
  it("UT-CRY-03: throws an error when decrypting an invalid format string", () => {
    expect(() => decrypt("invalidformat")).toThrowError(
      "Invalid cipher text format",
    );
    expect(() => decrypt("part1:part2")).toThrowError(
      "Invalid cipher text format",
    );
    expect(() => decrypt("part1:part2:part3:part4")).toThrowError(
      "Invalid cipher text format",
    );
  });
  ...
  it("UT-CRY-04: throws integrity check error for tampered ciphertext", () => {
    ...
    const tamperedCipher =
      originalCipher.substring(0, originalCipher.length - 1) +
      (originalCipher.endsWith("0") ? "1" : "0");
