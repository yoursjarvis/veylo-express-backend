...
describe("MediaRepository", () => {
  it("should update user avatar", async () => {
    prismaMock.user.update.mockResolvedValueOnce({ id: "user-1", image: "avatar-url" });
    const result = await mediaRepository.updateUserAvatar("user-1", "avatar-url");
  ...
  it("should find org member", async () => {
    prismaMock.member.findFirst.mockResolvedValueOnce({ id: "member-1", userId: "user-1", organizationId: "org-1" });
  ...
  it("should find workspace member", async () => {
    prismaMock.workspaceMember.findFirst.mockResolvedValueOnce({ id: "ws-member-1" });
    const result = await mediaRepository.findWorkspaceMember("ws-1", "user-1", "org-1");
  ...
  it("should update workspace icon", async () => {
    prismaMock.workspace.update.mockResolvedValueOnce({ id: "ws-1", icon: "icon-url" });
    const result = await mediaRepository.updateWorkspaceIcon("ws-1", "icon-url");
  ...
  it("should update project icon", async () => {
    prismaMock.project.update.mockResolvedValueOnce({ id: "proj-1", icon: "icon-url" });
    const result = await mediaRepository.updateProjectIcon("proj-1", "icon-url");
