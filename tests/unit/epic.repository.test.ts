...
describe("EpicRepository", () => {
  ...
  it("should find epics by projectId", async () => {
    prismaMock.epic.findMany.mockResolvedValueOnce([
  ...
  it("should find an epic by id", async () => {
    prismaMock.epic.findUnique.mockResolvedValueOnce({
      id: "epic-1",
      title: "Epic",
    });
    const result = await epicRepository.findById("epic-1");
    expect(prismaMock.epic.findUnique).toHaveBeenCalledWith({
      where: { id: "epic-1" },
    });
    expect(result?.id).toBe("epic-1");
  });
  ...
  it("should update an epic", async () => {
    prismaMock.epic.update.mockResolvedValueOnce({
      id: "epic-1",
      title: "Updated Epic",
    });
    const result = await epicRepository.update("epic-1", {
      title: "Updated Epic",
    });
  ...
  it("should find an epic by id with trashed", async () => {
    prismaMock.epic.findUniqueWithTrashed.mockResolvedValueOnce({
      id: "epic-1",
      title: "Trashed Epic",
    });
    const result = await epicRepository.findByIdWithTrashed("epic-1");
    expect(prismaMock.epic.findUniqueWithTrashed).toHaveBeenCalledWith({
      where: { id: "epic-1" },
    });
  ...
  it("should restore an epic", async () => {
    ...
    expect(prismaMock.epic.restore).toHaveBeenCalledWith({
      where: { id: "epic-1" },
    });
    expect(result.id).toBe("epic-1");
  });
  ...
  it("should force delete an epic", async () => {
    ...
    expect(prismaMock.epic.forceDelete).toHaveBeenCalledWith({
      where: { id: "epic-1" },
    });
    expect(result.id).toBe("epic-1");
  });
