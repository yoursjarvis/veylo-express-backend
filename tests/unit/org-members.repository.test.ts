...
describe("OrgMembersRepository", () => {
  it("should find caller member and target member", async () => {
    ...
    expect(
      await orgMembersRepository.findCallerMember("org-1", "user-1"),
    ).toEqual({ id: "m-caller" });
    expect(
      await orgMembersRepository.findTargetMember("org-1", "user-2"),
    ).toEqual({ id: "m-target" });
  });
  ...
  it("should ban and unban user", async () => {
    ...
    expect(await orgMembersRepository.banUser("user-1", "spamming")).toEqual({
      id: "user-1",
      banned: true,
    });
    expect(await orgMembersRepository.unbanUser("user-1")).toEqual({
      id: "user-1",
      banned: false,
    });
  });
  ...
  it("should delete sessions and find user by id", async () => {
    ...
    expect(await orgMembersRepository.deleteSessionsByUserId("user-1")).toEqual(
      { count: 2 },
    );
    expect(await orgMembersRepository.findUserById("user-1")).toEqual({
      id: "user-1",
    });
  });
  ...
  it("should create session", async () => {
    ...
    expect(await orgMembersRepository.createSession(data)).toEqual({
      id: "sess-1",
      ...data,
    });
  });
  ...
  it("should find invitation by id, pending invitations, and invitation in org", async () => {
    ...
    expect(await orgMembersRepository.findInvitationById("invite-1")).toEqual({
      id: "invite-1",
    });
    expect(
      await orgMembersRepository.findPendingInvitations("org-1"),
    ).toHaveLength(1);
    expect(
      await orgMembersRepository.findInvitationInOrg("invite-1", "org-1"),
    ).toEqual({ id: "invite-1" });
  });
});
