...
describe("LabelService", () => {
  ...
  describe("createLabel", () => {
    it("creates a label successfully", async () => {
      ...
      labelRepositoryMock.create.mockResolvedValueOnce({
        id: "l1",
        name: "Bug",
        color: "#ff0000",
      });

      const result = await labelService.createLabel("proj-1", "org-1", {
        name: "Bug",
        color: "#ff0000",
      });

      expect(labelRepositoryMock.findByNameAndProjectId).toHaveBeenCalledWith(
        "Bug",
        "proj-1",
      );
    ...
    it("throws BadRequestException if label name already exists", async () => {
      labelRepositoryMock.findByNameAndProjectId.mockResolvedValueOnce({
        id: "l-existing",
      });

      await expect(
        labelService.createLabel("proj-1", "org-1", {
          name: "Bug",
          color: "#ff0000",
        }),
  ...
  describe("getLabels", () => {
    it("returns labels for a project", async () => {
      labelRepositoryMock.findByProjectId.mockResolvedValueOnce([
        { id: "l1" },
        { id: "l2" },
      ]);

      const result = await labelService.getLabels("proj-1");
      expect(labelRepositoryMock.findByProjectId).toHaveBeenCalledWith(
        "proj-1",
      );
  ...
  describe("updateLabel", () => {
    it("updates a label when no duplicate name exists", async () => {
      ...
      labelRepositoryMock.update.mockResolvedValueOnce({
        id: "l1",
        name: "Feature",
      });
      ...
      expect(labelRepositoryMock.findDuplicateName).toHaveBeenCalledWith(
        "proj-1",
        "Feature",
        "l1",
      );
      expect(labelRepositoryMock.update).toHaveBeenCalledWith("l1", {
        name: "Feature",
        color: "#00ff00",
      });
    ...
    it("throws BadRequestException if duplicate name exists when updating", async () => {
      labelRepositoryMock.findDuplicateName.mockResolvedValueOnce({
        id: "l-other",
      });
  ...
  describe("restoreLabel", () => {
    it("throws NotFoundException if label not found during restore", async () => {
      ...
      await expect(labelService.restoreLabel("l-missing")).rejects.toThrow(
        "Label not found",
      );
    ...
    it("restores a soft-deleted label", async () => {
      labelRepositoryMock.findByIdWithTrashed.mockResolvedValueOnce({
        id: "l1",
      });
  ...
  describe("forceDeleteLabel", () => {
    it("throws NotFoundException if label not found during force delete", async () => {
      ...
      await expect(labelService.forceDeleteLabel("l-missing")).rejects.toThrow(
        "Label not found",
      );
