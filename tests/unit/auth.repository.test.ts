...
describe("AuthRepository", () => {
  it("should find user by email", async () => {
    prismaMock.user.findUnique.mockResolvedValueOnce({ id: "user-1", email: "test@example.com" });
  ...
  it("should mark login success", async () => {
    const now = new Date();
    prismaMock.user.update.mockResolvedValueOnce({ id: "user-1", lastLoginAt: now });
  ...
  it("should mark login failure for non-existent user best effort", async () => {
    ...
    const result = await authRepository.markLoginFailure("nonexistent@example.com");
    expect(result).toBeUndefined();
  });

  it("should mark login failure without locking if attempts < 5", async () => {
    const user = { id: "user-1", email: "test@example.com", failedLoginAttempts: 2 };
    prismaMock.user.findUnique.mockResolvedValueOnce(user);
    prismaMock.user.update.mockResolvedValueOnce({ id: "user-1", failedLoginAttempts: 3 });
  ...
  it("should lock user if attempts >= 5", async () => {
    const user = { id: "user-1", email: "test@example.com", failedLoginAttempts: 4 };
    prismaMock.user.findUnique.mockResolvedValueOnce(user);
    prismaMock.user.update.mockResolvedValueOnce({ id: "user-1", failedLoginAttempts: 5 });
  ...
  it("should update session metadata by token", async () => {
    ...
    const result = await authRepository.updateSessionMetadataByToken("token-123", meta);
  ...
  it("should revoke session by id for user", async () => {
    prismaMock.session.updateMany.mockResolvedValueOnce({ count: 1 });
    const now = new Date();
    const result = await authRepository.revokeSessionByIdForUser("sess-1", "user-1", now);
    expect(prismaMock.session.updateMany).toHaveBeenCalledWith({
