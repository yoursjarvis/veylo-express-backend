...
describe("NotificationService", () => {
  ...
  it("should get notifications, mark read, and mark all read", async () => {
    vi.mocked(notificationRepository.getNotifications).mockResolvedValueOnce([
      { id: "n-1" } as unknown,
    ]);
    expect(
    ...
    vi.mocked(notificationRepository.findById).mockResolvedValueOnce({
      id: "n-1",
      recipientId: "user-1",
    } as unknown);
    vi.mocked(notificationRepository.markAsRead).mockResolvedValueOnce({
      id: "n-1",
      isRead: true,
    } as unknown);
    expect(
      await notificationService.markNotificationAsRead("user-1", "n-1"),
    ).toEqual({ id: "n-1", isRead: true });

    // NotFoundException cases
    vi.mocked(notificationRepository.findById).mockResolvedValueOnce(null);
    await expect(
      notificationService.markNotificationAsRead("user-1", "n-1"),
    ...
    vi.mocked(notificationRepository.markAllAsRead).mockResolvedValueOnce({
      count: 5,
    } as unknown);
    expect(
      await notificationService.markAllUserNotificationsAsRead("user-1"),
    ).toEqual({ count: 5 });
  });
  ...
  it("should dispatch slack webhooks", async () => {
    ...
    vi.mocked(
      notificationRepository.findActiveSlackWebhooks,
  ...
  it("should handle task created notifications", async () => {
    ...
    vi.mocked(
      notificationRepository.findTaskForNotification,
    ...
    expect(notificationRepository.findTaskForNotification).toHaveBeenCalledWith(
      "task-1",
    );
  });
  ...
  it("should handle task updated notifications", async () => {
    ...
    vi.mocked(notificationRepository.findUserName).mockResolvedValueOnce({
      name: "Bob",
    });
    vi.mocked(
      notificationRepository.findActiveSlackWebhooks,
  ...
  it("should handle comment added notifications and mention parser", async () => {
    ...
    vi.mocked(
      notificationRepository.findCommentForNotification,
    ).mockResolvedValueOnce(comment as unknown);
  ...
  it("should handle project added notifications", async () => {
    ...
    vi.mocked(
      notificationRepository.findProjectForNotification,
    ).mockResolvedValueOnce(project as unknown);
    vi.mocked(notificationRepository.findUserName).mockResolvedValueOnce({
      name: "Bob",
    });

    await notificationService.handleAddedToProject("proj-1", "user-1", [
      "user-2",
    ]);
    expect(
      notificationRepository.findProjectForNotification,
  ...
  it("should handle comment reaction notifications", async () => {
    ...
    vi.mocked(
      notificationRepository.findReactionForNotification,
    ).mockResolvedValueOnce(reaction as unknown);

    await notificationService.handleCommentReaction("reaction-1");
    expect(
      notificationRepository.findReactionForNotification,
    ).toHaveBeenCalledWith("reaction-1");
  });
  ...
  it("should skip when no task/comment/reaction found", async () => {
    ...
    await notificationService.handleAddedToProject("proj-missing", "user-1", [
      "user-2",
    ]);
  });
  ...
  it("should handle description mentions with all fresh mentions matched", async () => {
    ...
    vi.mocked(notificationRepository.getUserPreferences).mockResolvedValue(
      null,
    );
  ...
  it("should handle slack webhook dispatch errors gracefully", async () => {
    vi.mocked(
      notificationRepository.findActiveSlackWebhooks,
    ).mockResolvedValueOnce([{ url: "https://slack.com", channel: "general" }]);
    // Make fetch throw
    vi.stubGlobal(
      "fetch",
  ...
  it("should notify new and old assignee on handleTaskUpdated with assignee change", async () => {
    ...
    await notificationService.handleTaskUpdated(
      "task-1",
      "user-updater",
      oldTask,
    );
  ...
  it("should send slack message on handleTaskUpdated when status and assignee change", async () => {
    ...
    expect(notificationRepository.findActiveSlackWebhooks).toHaveBeenCalledWith(
      "proj-1",
    );
  });
  ...
  it("should catch error in handleAddedToProject gracefully", async () => {
    vi.mocked(
      notificationRepository.findProjectForNotification,
    ).mockRejectedValueOnce(new Error("DB error in handleAddedToProject"));
  ...
  it("should catch error in handleCommentReaction gracefully", async () => {
    vi.mocked(
      notificationRepository.findReactionForNotification,
    ).mockRejectedValueOnce(new Error("DB error in handleCommentReaction"));
  ...
  it("should catch error in handleCommentAdded gracefully", async () => {
    vi.mocked(
      notificationRepository.findCommentForNotification,
    ).mockRejectedValueOnce(new Error("DB error in handleCommentAdded"));
