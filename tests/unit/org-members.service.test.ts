...
describe("OrgMembersService", () => {
  ...
  describe("verifyAdminAccess", () => {
    it("should throw ForbiddenException if caller is not in organization", async () => {
      vi.mocked(orgMembersRepository.findCallerMember).mockResolvedValueOnce(
        null,
      );
      await expect(
    ...
    it("should bypass permission check if self-modifying and allowSelf is true", async () => {
      ...
      const result = await orgMembersService.verifyAdminAccess(
        "org-1",
        "user-1",
        "user-1",
        "update",
    ...
    it("should throw ForbiddenException if RBAC authorization fails", async () => {
      ...
      await expect(
        orgMembersService.verifyAdminAccess(
          "org-1",
          "user-1",
          "user-2",
          "update",
    ...
    it("should throw NotFoundException if target user is not in organization", async () => {
      ...
      vi.mocked(orgMembersRepository.findTargetMember).mockResolvedValueOnce(
        null,
      );
    ...
    it("should throw ForbiddenException if admin attempts to modify another admin/owner", async () => {
      ...
      prisma.organization.findUnique = vi
        .fn()
        .mockResolvedValue({ ownerId: "user-owner" });
      prisma.userRoleAssignment.findFirst = vi
        .fn()
  ...
  describe("Operations", () => {
    it("should ban and unban members", async () => {
      const caller = { id: "m-1", userId: "user-1", organizationId: "org-1" };
      vi.mocked(orgMembersRepository.findCallerMember).mockResolvedValue(
        caller as unknown,
      );
      vi.mocked(rbacService.authorize).mockResolvedValue(true);
      vi.mocked(orgMembersRepository.findTargetMember).mockResolvedValue({
        id: "m-2",
      } as unknown);

      await orgMembersService.banMember("org-1", "user-1", "user-2", "reason");
      expect(orgMembersRepository.banUser).toHaveBeenCalledWith(
        "user-2",
        "reason",
      );
    ...
    it("should revoke sessions and get sessions", async () => {
      ...
      expect(orgMembersRepository.deleteSessionsByUserId).toHaveBeenCalledWith(
        "user-2",
      );

      prisma.session.findMany = vi
        .fn()
    ...
    it("should impersonate user (with Better Auth and manual fallback)", async () => {
      ...
      vi.mocked(auth.api.impersonateUser as unknown).mockResolvedValueOnce({
        sessionToken: "token",
      });
      const res1 = await orgMembersService.impersonateUser(
        "org-1",
        "user-1",
        "user-2",
        {},
      ...
      vi.mocked(auth.api.impersonateUser as unknown).mockRejectedValueOnce(
        new Error("Error"),
      );
      vi.mocked(orgMembersRepository.createSession).mockResolvedValueOnce({
        id: "sess-fallback",
      } as unknown);
      const res2 = await orgMembersService.impersonateUser(
        "org-1",
        "user-1",
        "user-2",
        {},
    ...
    it("should update photo", async () => {
      ...
      vi.mocked(mediaService.uploadAvatar).mockResolvedValueOnce({
        url: "https://newphoto.png",
      } as unknown);
      const url = await orgMembersService.updatePhoto(
        "org-1",
        "user-1",
        "user-2",
        {
    ...
    it("should list members", async () => {
      ...
      vi.mocked(rbacService.authorize).mockResolvedValue(true);
      vi.mocked(orgMembersRepository.findMembers).mockResolvedValue([
        { id: "m-1" },
        { id: "m-2" },
      ] as unknown);

      const result = await orgMembersService.getMembers("org-1", "user-1", {
        limit: 1,
      });
    ...
    it("should get public details of active invitations", async () => {
      ...
      vi.mocked(orgMembersRepository.findInvitationById).mockResolvedValueOnce(
      ...
      await expect(
        orgMembersService.getInvitationPublic("invite-1"),
    ...
    it("should get pending invitations", async () => {
      ...
      vi.mocked(rbacService.authorize).mockResolvedValue(true);
      vi.mocked(
        orgMembersRepository.findPendingInvitations,
      ).mockResolvedValueOnce([{ id: "inv-1" }] as unknown);

      expect(
        await orgMembersService.getPendingInvitations("org-1", "user-1"),
      ).toHaveLength(1);
    });

    it("should invite member", async () => {
      vi.mocked(auth.api.createInvitation as unknown).mockResolvedValueOnce({
        id: "inv-1",
      });
      prisma.invitation.update = vi.fn().mockResolvedValueOnce({});

      const invite = await orgMembersService.inviteMember(
        "org-1",
        "guest@example.com",
        "member",
        ["proj-1"],
    ...
    it("should bulk invite members via CSV/Excel", async () => {
      ...
      const resultCsv = await orgMembersService.bulkInvite(
        "org-1",
        "user-1",
        {
          buffer: csvBuffer,
    ...
    it("should cancel/revoke invitation", async () => {
      ...
      vi.mocked(auth.api.cancelInvitation as unknown).mockResolvedValueOnce({
        success: true,
      });
      ...
      await expect(
        orgMembersService.revokeInvitation(
          "org-1",
          "user-1",
          "inv-missing",
          {},
    ...
    it("should throw NotFoundException on resendInvitation if invitation not found", async () => {
      ...
      await expect(
        orgMembersService.resendInvitation(
          "org-1",
          "user-1",
          "inv-missing",
          {},
    ...
    it("should handle bulk invite with missing email in CSV row and invite failure", async () => {
      ...
      vi.mocked(auth.api.createInvitation as unknown).mockRejectedValueOnce(
        new Error("Invite error"),
      ); // first valid invite fails

      const result = await orgMembersService.bulkInvite(
        "org-1",
        "user-1",
        {
          buffer: csvBuffer,
