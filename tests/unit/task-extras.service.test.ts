...
describe("TaskExtrasService", () => {
  ...
  describe("createStatus", () => {
    it("throws BadRequestException if status name exists", async () => {
      taskExtrasRepositoryMock.findStatusByNameAndProjectId.mockResolvedValueOnce(
        { id: "s-existing" },
      );
      await expect(
        taskExtrasService.createStatus("proj-1", "org-1", {
          name: "Todo",
          category: "todo",
          order: 0,
        }),
    ...
    it("creates status with defaults", async () => {
      ...
      taskExtrasRepositoryMock.createStatus.mockResolvedValueOnce({
        id: "s-1",
      });
  ...
  describe("updateStatus", () => {
    it("updates status", async () => {
      taskExtrasRepositoryMock.updateStatus.mockResolvedValueOnce({
        id: "s-1",
      });
      const result = await taskExtrasService.updateStatus("s-1", {
        name: "New Name",
      });
      expect(result).toEqual({ id: "s-1" });
    });
  ...
  describe("deleteStatus", () => {
    it("throws if tasks are using the status", async () => {
      ...
      await expect(taskExtrasService.deleteStatus("s-1")).rejects.toThrow(
  ...
  describe("restoreStatus / forceDeleteStatus", () => {
    it("throws NotFoundException if status not found for restore", async () => {
      taskExtrasRepositoryMock.findStatusByIdWithTrashed.mockResolvedValueOnce(
        null,
      );
      await expect(
        taskExtrasService.restoreStatus("s-missing"),
    ...
    it("restores a deleted status", async () => {
      ...
      taskExtrasRepositoryMock.restoreStatus.mockResolvedValueOnce({
        id: "s-1",
      });
    ...
    it("throws NotFoundException if status not found for force delete", async () => {
      ...
      await expect(
        taskExtrasService.forceDeleteStatus("s-missing"),
      ).rejects.toThrow("Status not found");
    });
    ...
    it("force deletes a status", async () => {
      ...
      taskExtrasRepositoryMock.forceDeleteStatus.mockResolvedValueOnce({
        id: "s-1",
      });
  ...
  describe("createSubtask", () => {
    it("throws NotFoundException if parent task not found", async () => {
      ...
      await expect(
        taskExtrasService.createSubtask(
          "task-missing",
          "org-1",
          { title: "Sub" },
          "user-1",
    ...
    it("creates a subtask successfully", async () => {
      ...
      taskExtrasRepositoryMock.createTaskActivity.mockResolvedValueOnce(
        undefined,
      );

      const result = await taskExtrasService.createSubtask(
        "task-1",
        "org-1",
        { title: "Sub" },
        "user-1",
  ...
  describe("updateSubtask", () => {
    ...
    it("updates subtask without status change (no activity logged for status)", async () => {
      taskExtrasRepositoryMock.updateSubtask.mockResolvedValueOnce({
        id: "sub-1",
      });

      await taskExtrasService.updateSubtask(
        subtask,
        { title: "Updated" },
        "user-1",
      );
      ...
      expect(
        taskExtrasRepositoryMock.createTaskActivity,
  ...
  describe("deleteSubtask", () => {
    it("deletes subtask and logs activity", async () => {
      ...
      expect(taskExtrasRepositoryMock.deleteSubtask).toHaveBeenCalledWith(
        "sub-1",
      );
      expect(taskExtrasRepositoryMock.createTaskActivity).toHaveBeenCalledWith(
  ...
  describe("createComment", () => {
    it("creates comment and triggers notification", async () => {
      taskExtrasRepositoryMock.createComment.mockResolvedValueOnce({
        id: "comment-1",
      });
      taskExtrasRepositoryMock.createTaskActivity.mockResolvedValueOnce(
        undefined,
      );
      notificationServiceMock.handleCommentAdded.mockResolvedValueOnce(
        undefined,
      );

      const result = await taskExtrasService.createComment(
        "task-1",
        "org-1",
        { content: "Hello!" },
        "user-1",
      ...
      expect(notificationServiceMock.handleCommentAdded).toHaveBeenCalledWith(
        "comment-1",
        "user-1",
      );
    });
  ...
  describe("deleteComment", () => {
    it("deletes comment when user is the author", async () => {
      ...
      expect(taskExtrasRepositoryMock.deleteComment).toHaveBeenCalledWith(
        "comment-1",
      );
    ...
    it("throws ForbiddenException when non-author lacks permission", async () => {
      taskRepositoryMock.findProjectById.mockResolvedValueOnce({
        id: "proj-1",
        workspaceId: "ws-1",
      });
  ...
  describe("restoreComment / forceDeleteComment", () => {
    it("throws NotFoundException if comment not found for restore", async () => {
      taskExtrasRepositoryMock.findCommentByIdWithTrashed.mockResolvedValueOnce(
        null,
      );
      await expect(
        taskExtrasService.restoreComment("c-missing"),
    ...
    it("restores a comment", async () => {
      ...
      taskExtrasRepositoryMock.restoreComment.mockResolvedValueOnce({
        id: "c-1",
      });
    ...
    it("throws NotFoundException if comment not found for force delete", async () => {
      ...
      await expect(
        taskExtrasService.forceDeleteComment("c-missing"),
      ).rejects.toThrow("Comment not found");
    });
    ...
    it("force deletes a comment", async () => {
      ...
      taskExtrasRepositoryMock.forceDeleteComment.mockResolvedValueOnce({
        id: "c-1",
      });
  ...
  describe("updateComment", () => {
    it("throws NotFoundException if comment not found", async () => {
      ...
      await expect(
        taskExtrasService.updateComment(
          "c-missing",
          { content: "Updated" },
          "user-1",
        ),
    ...
    it("throws ForbiddenException if user is not the author", async () => {
      taskExtrasRepositoryMock.findCommentById.mockResolvedValueOnce({
        id: "c-1",
        userId: "user-author",
      });
      await expect(
    ...
    it("updates comment when user is the author", async () => {
      ...
      taskExtrasRepositoryMock.updateComment.mockResolvedValueOnce({
        id: "c-1",
        content: "Updated",
      });

      const result = await taskExtrasService.updateComment(
        "c-1",
        { content: "Updated" },
        "user-1",
      );
  ...
  describe("createCustomField", () => {
    it("throws BadRequestException if field name exists", async () => {
      taskExtrasRepositoryMock.findCustomFieldByName.mockResolvedValueOnce({
        id: "cf-existing",
      });
      await expect(
        taskExtrasService.createCustomField("proj-1", "org-1", {
          name: "Priority",
          type: "text",
        }),
      ).rejects.toThrow(
    ...
    it("creates custom field with options", async () => {
      ...
      taskExtrasRepositoryMock.createCustomField.mockResolvedValueOnce({
        id: "cf-1",
      });

      const result = await taskExtrasService.createCustomField(
        "proj-1",
        "org-1",
        {
          name: "Size",
  ...
  describe("getCustomFields", () => {
    it("returns custom fields for a project", async () => {
      taskExtrasRepositoryMock.findCustomFieldsByProjectId.mockResolvedValueOnce(
        [{ id: "cf-1" }],
      );
  ...
  describe("deleteCustomField", () => {
    it("deletes a custom field", async () => {
      taskExtrasRepositoryMock.deleteCustomField.mockResolvedValueOnce(
        undefined,
      );
      await taskExtrasService.deleteCustomField("cf-1");
      expect(taskExtrasRepositoryMock.deleteCustomField).toHaveBeenCalledWith(
        "cf-1",
      );
    });
  });
  ...
  describe("restoreCustomField / forceDeleteCustomField", () => {
    it("throws NotFoundException for restore if field not found", async () => {
      taskExtrasRepositoryMock.findCustomFieldByIdWithTrashed.mockResolvedValueOnce(
        null,
      );
      await expect(
        taskExtrasService.restoreCustomField("cf-missing"),
    ...
    it("restores a custom field", async () => {
      ...
      taskExtrasRepositoryMock.restoreCustomField.mockResolvedValueOnce({
        id: "cf-1",
      });
    ...
    it("throws NotFoundException for force delete if field not found", async () => {
      ...
      await expect(
        taskExtrasService.forceDeleteCustomField("cf-missing"),
      ).rejects.toThrow("Custom field not found");
    });
    ...
    it("force deletes a custom field", async () => {
      ...
      taskExtrasRepositoryMock.forceDeleteCustomField.mockResolvedValueOnce({
        id: "cf-1",
      });
  ...
  describe("getReactionUsers", () => {
    it("returns reaction users for a comment and emoji", async () => {
      taskExtrasRepositoryMock.findCommentReactions.mockResolvedValueOnce([
        { id: "r-1" },
      ]);
      const result = await taskExtrasService.getReactionUsers(
        "comment-1",
        "👍",
      );
      expect(result).toHaveLength(1);
  ...
  describe("toggleCommentReaction", () => {
    it("removes existing reaction (toggle off)", async () => {
      taskExtrasRepositoryMock.findCommentReaction.mockResolvedValueOnce({
        id: "r-1",
      });
      taskExtrasRepositoryMock.deleteCommentReaction.mockResolvedValueOnce(
        undefined,
      );
      ...
      expect(
        taskExtrasRepositoryMock.deleteCommentReaction,
    ...
    it("creates new reaction (toggle on) and triggers notification", async () => {
      ...
      taskExtrasRepositoryMock.createCommentReaction.mockResolvedValueOnce({
        id: "r-new",
      });
      notificationServiceMock.handleCommentReaction.mockResolvedValueOnce(
        undefined,
      );
      ...
      expect(
        notificationServiceMock.handleCommentReaction,
      ).toHaveBeenCalledWith("r-new");
    });
  });
});
